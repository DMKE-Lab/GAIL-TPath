...
class Trainer(object):
    ...
    def initialize(self, restore=None, sess=None):
        ...
        self.per_example_loss, self.per_example_logits, self.action_idx, self.tim_idx = self.agent(
            self.candidate_relation_sequence, self.candidate_tim_sequence,self.candidate_entity_sequence, self.entity_sequence,self.input_path,self.input_path_tim,self.query_relation,self.query_tim, self.range_arr, self.first_state_of_test, self.path_length)
        ...
        with tf.variable_scope("policy_steps_unroll") as scope:
            ...
            self.test_loss, test_state,self.test_logits, self.test_action_idx, self.test_tim_idx, self.chosen_relation, self.chosen_tim = self.agent.step(self.next_relations, self.next_tims, self.next_entities, formated_state, self.prev_relation,self.prev_tim, self.query_embedding, self.query_embedding_tim,self.current_entities, self.input_path[0], self.input_path_tim[0], self.range_arr, self.first_state_of_test)
    ...
    def initialize_pretrained_embeddings(self, sess):
        if self.pretrained_embeddings_action != '':
            ...
            _ = sess.run((self.agent.relation_embedding_init),feed_dict={self.agent.action_embedding_placeholder: embeddings})
        if self.pretrained_embeddings_entity != '':
            embeddings = np.loadtxt(open(self.pretrained_embeddings_entity))
            _ = sess.run((self.agent.entity_embedding_init),feed_dict={self.agent.entity_embedding_placeholder: embeddings})
        if self.pretrained_embeddings_tim != '':
            embeddings = np.loadtxt(open(self.pretrained_embeddings_tim))
            _ = sess.run((self.agent.tim_embedding_init),feed_dict={self.agent.tim_embedding_placeholder: embeddings})
    ...
    def reshape_reward(self, rewards):
        reshaped_reward = np.zeros([rewards.shape[0], 1])
    ...
    def gpu_io_setup(self):
        ...
        feeds =  [self.first_state_of_test] + self.candidate_relation_sequence+ \
                 self.candidate_tim_sequence + self.candidate_entity_sequence + self.input_path\
                 + self.input_path_tim + [self.query_relation] + [self.query_tim]\
                 + [self.final_reward] + [self.range_arr] + self.entity_sequence
    ...
    def pre_train(self, sess):
        with open(self.input_dir + 'demo_path.txt') as f:
            ...
            real_inputs_all = real_inputs_all.astype('float32')
        with open(self.input_dir + 'demo_path.txt') as f:
    ...
    def train(self, sess):
        ...
        for episode in self.train_environment.get_episodes():
            ...
            for i in range(self.path_length):
                ...
                candidate_paths.append(state['next_relations'][::20][0][action_idx])
            ...
            logger.info("batch_counter: {0:4d}, num_hits: {1:7.4f}, avg. reward per batch {2:7.4f}, "
                        ...
                        format(self.batch_counter, np.sum(rewards), avg_reward, num_ep_correct,(num_ep_correct / self.batch_size),train_loss))
    ...
    def test(self, sess, beam=False, print_paths=False, save_model = True, auc = False):
        ...
        for episode in tqdm(self.test_environment.get_episodes()):
            ...
            for i in range(self.path_length):
                ...
                loss,agent_mem, test_scores, test_action_idx,test_tim_idx, chosen_relation,chosen_tim = sess.run( [self.test_loss, self.test_state, self.test_logits, self.test_action_idx,self.test_tim_idx,self.chosen_relation,self.chosen_tim], feed_dict=feed_dict)
                if beam:
