...
class Agent(object):
    ...
    def __init__(self, params):
        ...
        with tf.variable_scope("relation_lookup_table"):
            ...
            self.relation_lookup_table = tf.get_variable("relation_lookup_table",shape=[self.relation_vocab_size, 2 * self.embedding_size],dtype=tf.float32,initializer=tf.contrib.layers.xavier_initializer(),trainable=self.train_relations)
        ...
        with tf.variable_scope("tim_lookup_table"):
            ...
            self.tim_lookup_table = tf.get_variable("tim_lookup_table",shape=[self.tim_vocab_size, 2 * self.embedding_size],dtype=tf.float32,initializer=tf.contrib.layers.xavier_initializer(),trainable=self.train_tims)
        ...
        with tf.variable_scope("entity_lookup_table"):
            ...
            self.entity_lookup_table = tf.get_variable("entity_lookup_table",shape=[self.entity_vocab_size, 2 * self.entity_embedding_size],dtype=tf.float32,initializer=self.entity_initializer,trainable=self.train_entities)
    ...
    def step(self, next_relations, next_tims,next_entities, prev_state, prev_relation, prev_tim,query_embedding, query_embedding_tim, current_entities,label_action, label_tim,range_arr, first_step_of_test):
    ...
    def __call__(self, candidate_relation_sequence, candidate_tim_sequence, candidate_entity_sequence, current_entities,path_label, path_label_tim, query_relation, query_tim, range_arr, first_step_of_test, T=3, entity_sequence=0):
        ...
        with tf.variable_scope("policy_steps_unroll") as scope:
            for t in range(T):
                ...
                loss, state, logits, act_idx,ti_idx, chosen_relation ,chosen_tim= self.step(next_possible_relations, next_possible_tims,next_possible_entities,state, prev_relation, prev_tim, query_embedding,query_embedding_tim,current_entities_t,label_action=path_label_t,  label_tim=path_label_ti,range_arr=range_arr, first_step_of_test=first_step_of_test)
